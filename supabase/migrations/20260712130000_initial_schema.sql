...
as $$
  select case public.current_profile_role()
    ...
    when 'bursar' then required_permission in (
      'overview.view',
      'finance.view',
    ...
    when 'teacher' then required_permission in (
      'overview.view',
      'academics.view',
      'academics.attendance.write',
      'communications.view'
    )
    when 'transport' then required_permission in (
      'overview.view',
      'transport.view',
      'transport.status.write',
      'communications.view'
    )
    when 'student' then required_permission in ('overview.view', 'academics.view', 'communications.view')
    when 'parent' then required_permission in ('overview.view', 'academics.view', 'communications.view')
    else false
  end
$$;
...
drop policy if exists "Announcements write requires publish permission" on public.announcements;
