...
| DREEM-ACADEMICS-001 | Verified academic delivery | OpenEMIS structure plus evidence-led moderation | Named teaching ownership, collision-safe periods, independent assessment approval and report-card snapshots generated only from published evidence | Production schema and connected workspace implemented; automated checks pass | Two-person author/reviewer browser proof and family report access |
| DREEM-TRANSPORT-001 | Consent-controlled school transport | Fleet dispatch discipline plus AFAT interoperability boundary | Guardian authority precedes assignment; route, licence and capacity controls precede dispatch; immutable journey events can emit to AFAT without private database access | Production schema and connected workspace implemented; automated checks pass | Transport-manager/driver/parent role matrix and live journey proof |
