...
async function resolveIdentityMedia(reference: unknown): Promise<string | undefined> {
  ...
  const { data, error } = await client.storage.from(IDENTITY_BUCKET).createSignedUrl(value, SIGNED_URL_TTL_SECONDS);
...
export async function loadLearnerIdentity(studentId: string): Promise<LearnerIdentityProfile> {
  const client = requireClient();
  const studentResult = await client.from("students").select("id,school_id,matricule,full_name,class_name,photo_url,date_of_birth,sex").eq("id", studentId).single();
  if (studentResult.error) throw studentResult.error;
  const student = studentResult.data;

  const [linkResult, credentialResult] = await Promise.all([
    client.from("dreem_student_guardians").select("guardian_id,relationship,is_primary,can_collect,collector_label,collector_photo_url,collection_notes").eq("student_id", studentId),
    client.from("dreem_student_credentials").select("status,valid_until,card_number,card_version,issued_at").eq("student_id", studentId).order("issued_at", { ascending: false }).limit(1).maybeSingle(),
  ]);
  ...
  if (guardianIds.length) {
    const guardianResult = await client.from("dreem_guardians").select("id,full_name,phone,email,photo_url").in("id", guardianIds);
  ...
  const guardianRows = await Promise.all(links.map(async (link) => {
...
export async function updateGuardianIdentity(input: { studentId: string; guardianId: string; collectorLabel?: string; collectionNotes?: string; canCollect?: boolean }) {
  const client = requireClient();
  const update = await client.from("dreem_student_guardians").update({
  ...
  }).eq("student_id", input.studentId).eq("guardian_id", input.guardianId);
  if (update.error) throw update.error;
}

export async function revokeLearnerCredential(studentId: string, reason: string) {
  const client = requireClient();
  const cleanReason = reason.trim();
  if (!cleanReason) throw new Error("Give a reason before revoking a learner card.");
  const { data, error } = await client.rpc("dreem_revoke_student_credential", {
    p_student_id: studentId,
    p_reason: cleanReason,
    p_idempotency_key: `credential-revoke:${crypto.randomUUID()}`,
  });
  if (error) throw error;
  return Boolean(data);
}
