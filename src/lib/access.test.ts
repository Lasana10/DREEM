...
describe("authority-driven workspace access",()=>{
  it("does not make a title string a permission",()=>{
    const viewer={role:"teacher" as const,positionTitle:"Headmistress",authorityScopes:["academics"] as const};
  ...
  it("lets configured authority shape leadership access independently of legacy role",()=>{
    const viewer={role:"administrator" as const,positionTitle:"Director",authorityScopes:["institutional_leadership","admissions","communications"] as const};
  ...
  it("keeps finance collection and approval distinct",()=>{
    const collector={role:"bursar" as const,authorityScopes:["finance_collection"] as const};
    const reviewer={role:"accountant" as const,authorityScopes:["finance_approval","audit"] as const};
  ...
  it("uses authority to choose a natural first workspace",()=>{
    const transportLead={role:"administrator" as const,authorityScopes:["transport"] as const};
