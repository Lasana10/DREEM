...
export default function StudentWorkspace({ workspace, onRefresh }: { workspace: WorkspaceData; onRefresh: () => Promise<void> }) {
  ...
  if (!learner) return <div className="content"><section className="panel"><h2>Learner access needs attention</h2><p>The school must link this account to one learner before opening DREEM Student.</p></section></div>;
  ...
  const learnerClass = workspace.setup.classes.find(item => item.name === learner.className);
  const materials = workspace.academics.documents.filter(item => item.status === "approved" && (!item.classId || item.classId === learnerClass?.id));
  const now = new Date();
  ...
  const transportAssignment = workspace.transport.assignments.find(item => item.studentId === learner.id && item.status === "active");
  const trips = transportAssignment ? workspace.transport.trips.filter(item => item.routeId === transportAssignment.routeId && item.status !== "cancelled").slice(0,6) : [];
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (assignment.submissionMode === "offline") { setError("Hand this work to your teacher; this assignment is configured for classroom submission."); return; }
  ...
  return <div className="content student-workspace role-workspace">
    <section className="role-hero"><div><span className="eyebrow">MY SCHOOL DAY</span><h2>Hello, {learner.name.split(" ")[0]}</h2><p>{learner.className} · {due.length} work item{due.length===1?"":"s"} waiting</p></div></section>
    <nav className="workspace-tabs" aria-label="Student workspace">{tabs.map(item => <button key={item} className={tab === item ? "primary" : ""} aria-current={tab === item ? "page" : undefined} onClick={() => setTab(item)}>{item}</button>)}</nav>
    {error && <p className="form-status error" role="alert">{error}</p>}{message && <p className="form-status success" role="status">{message}</p>}
    {tab === "Today" && <section className="focus-card"><span className="eyebrow">TODAY · {today}</span><h3>Your school day</h3>{todayPeriods.map(item => <article className="document-row" key={item.id}><strong>{item.subjectName}</strong><span>{item.startsAt}–{item.endsAt} · {item.room || "Room to be confirmed"}</span></article>)}{!todayPeriods.length && <p>No scheduled lessons today.</p>}<h3>{due.length} assignment{due.length===1?"":"s"} awaiting your work</h3>{due.slice(0,5).map(item => <article className="document-row" key={item.id}><strong>{item.title}</strong><span>Due {new Date(item.dueAt).toLocaleString()}</span></article>)}<button onClick={() => setTab("Work")}>Open my work</button></section>}
    {tab === "Classes" && <><section className="panel"><h3>My timetable</h3>{[...periods].sort((a,b) => a.weekday-b.weekday || a.startsAt.localeCompare(b.startsAt)).map(item => <article className="document-row" key={item.id}><strong>{item.subjectName}</strong><span>{["", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"][item.weekday]} · {item.startsAt}–{item.endsAt}</span><small>{item.teacherName} · {item.room || "Room to be confirmed"}</small></article>)}{!periods.length && <p>Your school has not published a timetable for your class.</p>}</section><section className="panel"><h3>Learning materials released to my class</h3>{materials.map(item => <article className="document-row" key={item.id}><strong>{item.title}</strong><span>{item.documentType.replaceAll("_", " ")} · {item.language}</span><small>{item.fileName}</small></article>)}{!materials.length && <p>No approved class material has been released yet.</p>}</section></>}
    ...
    {tab === "Transport" && <section className="panel"><h3>My school transport</h3>{transportAssignment?<><article className="document-row"><strong>{transportAssignment.routeName}</strong><span>{transportAssignment.pickupStopName} → {transportAssignment.dropoffStopName}</span><small>Active assignment</small></article>{trips.map(item => <article className="document-row" key={item.id}><strong>{item.serviceDate} · {item.direction}</strong><span>{item.status.replaceAll("_", " ")}{item.scheduledDeparture ? ` · ${item.scheduledDeparture}` : ""}</span></article>)}</>:<p>No active school transport assignment is linked to you.</p>}</section>}
  </div>;
}
