...
export default function CareView({workspace,onRefresh}:{workspace:WorkspaceData;onRefresh:()=>Promise<void>}){
  ...
  return <div className="content">
    <section className="role-hero"><div><span className="eyebrow">CARE · TODAY</span><h2>{metrics.urgent?metrics.urgent+" urgent concern"+(metrics.urgent===1?"":"s")+" need attention":metrics.open+" active learner support case"+(metrics.open===1?"":"s")}</h2><p>Record the concern, assign the right person and follow up. Protected details stay restricted automatically.</p></div><div className="role-hero-status"><span className={"status-pill "+(metrics.urgent?"danger":"info")}><ShieldAlert size={14}/>{metrics.restricted} restricted</span></div></section>
