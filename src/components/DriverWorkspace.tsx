...
export default function DriverWorkspace({workspace,onRefresh}:{workspace:WorkspaceData;onRefresh:()=>Promise<void>}){
  ...
  useEffect(()=>{
    ...
    const connect=async()=>{
      setOnline(true);
      try{
        const result=await replayDriverOffline(workspace.viewer);
        if(active&&result.synced){setMessage(`${result.synced} saved journey event${result.synced===1?"":"s"} synced to school records.`);await onRefresh();}
        if(active&&result.tampered)setError(`${result.tampered} queued journey event failed its integrity check and was not sent.`);
      }catch(reason){if(active)setError(errorText(reason));}
    };
    const disconnect=()=>setOnline(false);
    if(navigator.onLine)prepareDriverOfflineContext().catch(()=>{});
    window.addEventListener("online",connect);window.addEventListener("offline",disconnect);
    return()=>{active=false;window.removeEventListener("online",connect);window.removeEventListener("offline",disconnect);};
  },[workspace.viewer,onRefresh]);
  ...
  async function submit(event:FormEvent<HTMLFormElement>){
    ...
    try{
      const result=await progressTransportTripResilient({tripId:trip.id,eventType:String(data.get("eventType")),stopId:String(data.get("stopId")||"")||undefined,studentId:String(data.get("studentId")||"")||undefined,note:String(data.get("note")||""),idempotencyKey:`driver-trip:${crypto.randomUUID()}`},workspace.viewer);
      if(result.queued)setMessage("Journey event saved safely on this device · awaiting sync.");
      else{setMessage("Journey update recorded in the school trip history.");await onRefresh();}
  ...
  return <div className="content role-workspace driver-workspace">
    <section className="page-intro"><div><span>DREEM DRIVER · TODAY</span><h2>Drive the assigned trip. Record what actually happens.</h2><p>You can operate trips assigned to your driver identity. Route setup, consent administration and gate release remain outside the Driver app.</p></div><div className="care-assurance">{online?<Wifi/>:<WifiOff/>}<span><strong>{online?"Connected to school":"Working offline"}</strong><small>{online?`${driver.name} · server-confirmed writes`:`${driver.name} · journey events can be saved on this device`}</small></span></div></section>
    ...
    <section className="panel lifecycle-panel"><div className="panel-title"><MapPinned/><div><span>DRIVER JOURNEY</span><h3>Dispatch → depart → stops → learner movement → complete</h3></div></div><div className="lifecycle-rail"><span><b>1</b>Receive assigned trip<small>Transport manager</small></span><span><b>2</b>Depart<small>Offline-safe</small></span><span><b>3</b>Record stop arrival / delay<small>Offline-safe</small></span><span><b>4</b>Board / alight learner<small>Offline-safe</small></span><span><b>5</b>Complete journey<small>Server validates on sync</small></span></div></section>
